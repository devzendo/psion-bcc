...
/* 05/03/97 MJG Added -p option to allow segments to be aligned on paragraph 
                boundaries 
*/
...
FORWARD void padmod P((struct modstruct *modptr, bool_t argpalign));
...
bool_pt argpalign;
{
	...
	*cptr = 0;
	...
	if (modptr->loadflag)
	{
		...
		/* -p option selects 16 byte (paragraph) instead of default 4
                   byte alignment */
		segsz[seg] = ld_roundup(segsz[seg], argpalign ? 16 : 4, bin_off_t);
		comsz[seg] = ld_roundup(comsz[seg], argpalign ? 16 : 4, bin_off_t);
#endif
		cptr += sizecount;
	    }
	}
...
PRIVATE void padmod(modptr, argpalign)
struct modstruct *modptr;
bool_t argpalign;
{
    bin_off_t count;
	...
	if ((size = ld_roundup(segpos[seg], argpalign ? 16 : 4, bin_off_t) - segpos[seg]) != 0)
	{
	    setseg(seg);
	    writenulls(size);
	    segpos[seg] = spos;
