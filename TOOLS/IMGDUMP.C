...
void hexdump(unsigned char *bufstart, unsigned char *buf, long buflen, unsigned char *bufend)
{ 
  if (buf+buflen > bufend) {
    fprintf(stderr,"*** Requested dump would exceed file length: dump truncated ***\n");
    buflen = bufend-buf;
  }
...
void imagefiletype(unsigned char *buf, long buflen)
{
  ...
  printf("Data dump:\n");
  hexdump(buf, buf+ifh->HeaderSizeBytes+(ifh->CodeParas * 16), ifh->InitialisedData, bufend);
...
unsigned char *buf;
  ...
  buf=malloc(filesize);
  if (buf == NULL) {
    fprintf(stderr, "imgdump: Can't allocate buffer of %ld bytes for %s.\n", filesize, filename);
    close(fd);
    return;
  }
  ...
  if (nread!=filesize) {
    fprintf(stderr, "imgdump: Only read %ld bytes of %s - should have read %ld.\n", nread, filename, filesize);
    return;
  }
