 ...
 */
static int peep_option = PEEP_ALL;	/* peehole optimisations */
...
static ADDRESS *g_shft P4 (OPCODE, op, ILEN, ilen, ADDRESS *, ap1, ADDRESS *, ap2)
{
	...
	if (i == 16L && ilen == IL4) {
	    ...
	    case op_asr:
		g_code (op_swap, IL0, ap2, NIL_ADDRESS);
		g_code (op_ext, IL2, ap2, NIL_ADDRESS);
		break;
	    case op_lsr:
		g_code (op_clr, IL2, ap2, NIL_ADDRESS);
		g_code (op_swap, IL0, ap2, NIL_ADDRESS);
...
static ADDRESS *g_assign P2 (const EXPR *, ep, FLAGS, flags)
{
	...
	case IL12:
		 ...
		 */
		/* get the value */
		mask = (UVAL) bitmask (ep->v.p[0]->v.bit.width);
		...
		if (ap1->mode == am_immed) {
		    ...
		    g_rotate (ap3, ep->etp, -(int) ep->v.p[0]->v.bit.offset,
			      tp_void, 0);
		}
