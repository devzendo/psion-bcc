...
static ADDRESS *mk_legal P3 (ADDRESS *, ap, FLAGS, flags, TYP *, tp)
{
	...
	if (flags & F_FREG) {
	    ...
	    g_code (op_fild, IL2, ap1, NIL_ADDRESS);
	    freeop (ap);
	    ap = float_register ();
	    return ap;
	}
...
/*KDW */
	...
	case bt_int32:
	    ap2 = float_register ();
	    g_code (op_fild, (ILEN) size, ap, NIL_ADDRESS);
	    return ap2;
	default:
	    break;
	}
...
static ADDRESS *g_fcall P2 (const EXPR *, ep, FLAGS, flags)
{
	...
	if (regsave_option & MEMBER (reg - 1)) {
	    g_stack (size);
	    size = 0L;
	    g_code (op_pop, IL2, mk_reg (reg - 1), NIL_ADDRESS);
	}
    }
...
static ADDRESS *g_cast P4 (ADDRESS *, ap, TYP *, tp1, TYP *, tp2, FLAGS, flags)
{
	...
	case bt_int16:
	    ...
	    case am_indx:
	    case am_indx2:
		g_code (op_fild, IL2, ap, NIL_ADDRESS);
		break;
	    }
	    freeop (ap);
	    ap = float_register ();
	...
	case bt_pointer32:
	    ...
	    g_code (op_fild, IL8, ap1, NIL_ADDRESS);
	    freeop (ap1);
	    freeop (ap);
	    ap = float_register ();
	    return mk_legal (ap, flags, tp2);
	...
	case bt_longdouble:
	    ...
	    ap = data_register ();
	    ap1 = mk_scratch (2L);
	    g_code (op_fistp, IL2, ap1, NIL_ADDRESS);
	...
	case bt_longdouble:
	    ...
	    ap = mdata_register ();
	    ap1 = mk_scratch (4L);
	    g_code (op_fistp, IL4, ap1, NIL_ADDRESS);
	    g_fcode (op_fwait, IL0, NIL_ADDRESS, NIL_ADDRESS);
	    g_code (op_mov, IL2, mk_low (ap1), mk_low (ap));
	    g_code (op_mov, IL2, mk_high (ap1), mk_high (ap));
	    freeop (ap1);
	...
	case bt_longdouble:
	    ...
	    ap = mdata_register ();
	    ap1 = mk_scratch (8L);
	    g_code (op_fistp, IL8, ap1, NIL_ADDRESS);
	    g_fcode (op_fwait, IL0, NIL_ADDRESS, NIL_ADDRESS);
	    g_code (op_mov, IL2, mk_low (ap1), mk_low (ap));
	    g_code (op_mov, IL2, mk_high (ap1), mk_high (ap));
	    freeop (ap1);
