...
void initsym P0 (void)
{
    ...
    {
	{(const CHAR *) "class", (size_t) 5, kw_class, NIL_WORD},
	{(const CHAR *) "private", (size_t) 7, kw_private, NIL_WORD},
	{(const CHAR *) "public", (size_t) 6, kw_public, NIL_WORD},
	{(const CHAR *) "restrict", (size_t) 8, kw_restrict, NIL_WORD},
	{(const CHAR *) "this", (size_t) 4, kw_this, NIL_WORD},
	{NIL_CHAR, (size_t) 0, (TOKEN) 0, NIL_WORD}
    };
 ...
 *  parameter 'p'.  If it is the expect token then it moves onto
...
#ifdef EXTENSION
/*
 *  is_visibility_specifier() - is token a visibility specifier
 *
 *  The is_visibility_specifier() routines "peeks" ahead in the input
 *  buffer to see if the visibility specifier tokens are followed
 *  by a ":".
 */
BOOL is_visibility_specifier P0 (void)
{
    switch (lastst) {
    case kw_public:
    case kw_private:
...
/*
 *  is_class_scope() - is identifier a class scope identifier.
 *
 *  The is_class_scope() routine "peeps" ahead in the input buffer to
 *  see if the identifier is followed by the "::" token.
 */
BOOL is_class_scope P0 (void)
{
    switch (lastst) {
    case tk_id:
	skip_space ();
	return ((bufcur[0] == (CHAR) ':') && (bufcur[1] == (CHAR) ':'));
    default:
	return FALSE;
    }
}
