...
static ADDRESS *as_fcall P_ ((const EXPR *, FLAGS, const CHAR *, int));
...
static SETVAL peep_option = PEEP_ALL;	/* peehole optimisations */
...
int     opt_remove_redundant = OPT_YES;
const CHAR *opt_peep_sequence = (const CHAR *) "";
int     opt_register_remap = OPT_YES;
int     opt_parallel_instructions = OPT_LEVEL1;
int     opt_3op = OPT_NO;
...
static BOOL is_op3_possible (EXPR *node)
{
	...
	&& (node->v.p[0]->nodetype == en_register)
	&& (is_address_register (node->v.p[0]->v.r))) {
	return TRUE;
    }
    /* look for indexnode *arn+(1) or *arn-(1) */
    if ((node->nodetype == en_ref)
	&& (node->v.p[0]->nodetype == en_add)
	&& ((node->v.p[0]->v.p[0]->nodetype == en_register)
	    && (is_address_register (node->v.p[0]->v.p[0]->v.r))
	    && (node->v.p[0]->v.p[1]->nodetype == en_icon)
    ...
    if ((node->nodetype == en_ref)
	&& (node->v.p[0]->nodetype == en_add)
	&& ((node->v.p[0]->v.p[1]->nodetype == en_register)
	    && (is_address_register (node->v.p[0]->v.p[1]->v.r))
	    && (node->v.p[0]->v.p[0]->nodetype == en_icon)
...
static void call_library_r0_r1 P1 (const CHAR *, lib_name)
{
    ...
    g_code (op_xcall, OP_INT, mk_strlab (sp->name), NIL_ADDRESS);
}
 ...
 */
static ADDRESS *mk_legal P3 (ADDRESS *, ap, FLAGS, flags, SIZE, size)
{
    ADDRESS *ap2, *ap3;
    OPCODE  OpSto, OpLd;
 ...
 */
static void g_immed P3 (OPCODE, op, long, i, ADDRESS *, ap)
{
    if (ap != NIL_ADDRESS) {
	if (!(isanyreg (ap)) /*||(ap->mode==am_freg) */ )
	    FATAL ((__FILE__, "g_immed", "immed-to-memory"));
    }
    if ((i >= -32768 && i <= 32767) || (opt_const_in_ram != OPT_NO)) {
...
static void g_uimmed P3 (OPCODE, op, IVAL, i, ADDRESS *, ap)
{
    ...
    if ((i >= 0 && i <= 0xFFFF) || (opt_const_in_ram != OPT_NO)) {
...
static ADDRESS *g_unary P3 (const EXPR *, ep, FLAGS, flags, OPCODE, op)
{
    ...
    case bt_ulong:
    case bt_pointer32:
	ap = g_expr (ep->v.p[0], (FLAGS) (op == op_not ? F_ALL | F_UNSIGNED : F_ALL));
    ...
    case bt_longdouble:
	if (op == op_negi) {
	    ap = g_expr (ep->v.p[0], (FLAGS) (F_ALL /*| F_VOL */ ));
...
static ADDRESS *g_add P3 (const EXPR *, ep, FLAGS, flags, OPCODE, op)
{
	...
	if (is_op3_possible (ep->v.p[0])
	    ...
	    ap1 = g_expr (ep->v.p[0], (FLAGS) (F_XREG));
	    ap2 = g_expr (ep->v.p[1], (FLAGS) (F_XREG));
	    if ((ap1->mode == am_ainc)
		|| (ap1->mode == am_adec)
		|| (ap1->mode == am_preinc)
		|| (ap1->mode == am_predec)
		...
		|| (ap2->mode == am_direct)
		|| (ap2->mode == am_immed)) {
		FATAL ((__FILE__, "g_add", "inconsistency int "));
	    }
	    validate (ap1);
	    freeop (ap2);
	    freeop (ap1);
		...
		ap1 = g_expr (ep->v.p[1], (FLAGS) (F_VOL | (flags & (F_XREG))));
	...
	if (is_op3_possible (ep->v.p[0])
	    && is_op3_possible (ep->v.p[1])) {
	    ap1 = g_expr (ep->v.p[0], (FLAGS) (F_FREG));
	    ap2 = g_expr (ep->v.p[1], (FLAGS) (F_FREG));
	    if ((ap1->mode == am_ainc)
		|| (ap1->mode == am_adec)
		|| (ap1->mode == am_preinc)
		|| (ap1->mode == am_predec)
		...
		|| (ap2->mode == am_direct)
		|| (ap2->mode == am_immed)) {
		FATAL ((__FILE__, "g_add", "inconsistency float "));
	    }
	    validate (ap1);
	    freeop (ap2);
	    freeop (ap1);
	    ap3 = temporary_register (F_FREG);
	    g_code3 (op, OP_FLOAT, ap2, ap1, ap3);
	    ap1 = ap3;
	} else {
	    if (ep->v.p[0]->nodetype == en_fcon) {
		ap1 = g_expr (ep->v.p[1], (FLAGS) (F_VOL | (flags & (F_FREG))));
		ap2 = g_expr (ep->v.p[0], F_ALL);
		if (op == op_subf)
		    op = op_subrf;
	    } else {
...
static ADDRESS *g_logic P3 (const EXPR *, ep, FLAGS, flags, OPCODE, op)
{
    ...
    if ((ep->v.p[0]->nodetype == en_register)
	&& (ep->v.p[1]->nodetype == en_register)) {
	switch (op) {
	case op_and:
	    op = op_and3;
	    break;
	...
	if (tst_iconst (ep->v.p[0])) {
	    ap1 = g_expr (ep->v.p[1], (FLAGS) (F_VOL | (F_XREG)));
	    ap2 = g_expr (ep->v.p[0], (FLAGS) (F_ALL | F_UNSIGNED));
	} else {
	    ap1 = g_expr (ep->v.p[0], (FLAGS) (F_VOL | (F_XREG)));
	    ap2 = g_expr (ep->v.p[1], (FLAGS) (F_ALL | F_UNSIGNED));
	}
	validate (ap1);		/* in case push occurred */
	g_code (op, OP_INT, ap2, ap1);
	freeop (ap2);
...
static ADDRESS *g_mul P2 (const EXPR *, ep, FLAGS, flags)
{
	...
	if (opt_true_long != OPT_NO) {
		...
		g_code3 (op_xor3, OP_INT, ap2, ap1, ap3);
	    ...
	    g_code (op_addi, OP_INT, ap3, ap1);
	    if (is_signed == FALSE) {
		/* get signum */
		g_code (op_negi, OP_INT, ap1, ap2);
		g_code (op_and, OP_INT, ap5, ap5);
...
{
    int     is_signed = TRUE;
    ADDRESS *ap1, *ap2, *ap3, *ap4, *ap5, *ap6;
...
static SIZE g_parms P1 (const EXPR *, plist)
{
    ...
    is_parameter++;
    for (size = 0; plist != NIL_EXPR; plist = plist->v.p[1]) {
 ...
 *
 */
static ADDRESS *as_fcall P4 (const EXPR *, ep, FLAGS, flags, const CHAR *, libnam1, int, type)
{
/*
 ...
 */
PRIVATE ADDRESS *g_conditionalload P4 (const EXPR *, ep, ADDRESS *, truesrc, ADDRESS *, falsesrc, int, type)
{
    OPCODE  op, op2;
    ADDRESS *ap1;
...
PRIVATE void g_switch_table P4 (const EXPR *, ep, struct swtab *, sw, UVAL, min_caselabel, UVAL, max_caselabel)
{
     ...
     */
    if (min_caselabel != 0) {
	g_immed (op_subi, min_caselabel, ap);
	/* sinc cmpi only reads 2 operands we use g_code3 and   */
	/* don't use dst ap, even if tszb is in fact a 2-operand instr. */
	/* makes the job for the peephole-optimizer easier */
...
{
#ifdef PROBES
    if ((probe_option) && (interrupt_option == OPT_NO)) {
	...
	if (size < 32768l) {
	    ...
	    g_code (op_tstb, OP_INT, ap, NIL_ADDRESS);
	} else {
	    ap2 = data_register (F_DREG);
	    ap->mode = am_indx2;
	    ap->sreg = ap->preg;
	    ap->u.offset = mk_const (0L);
	    g_immed (op_ldi, -size, ap2);
	    g_code (op_tstb, OP_INT, ap, NIL_ADDRESS);
	    freeop (ap2);
	}
    }
...
PRIVATE void g_epilogue P0 (void)
{
	...
	if (is_leaf_function) {
	    ...
	    /* Save only those registers wich are really used */
	...
	/* Mask out registers we do not want to save */
...
PRIVATE void g_flush P1 (SYM *, sp)
{
    put_literals ();
    if (sp) {
	put_cseg (alignment_of_type (sp->tp));
	put_name (sp);
    }
...
PRIVATE EXPR *g_transform P1 (EXPR *, ep)
{
	...
	if (ep->v.p[0]->nodetype == en_add) {
	    if ((ep->v.p[0]->v.p[1]->nodetype == en_icon)
...
static OPTENUM threelevelopts[] =
{
    {(const char *) "none", OPT_NONE},
    {(const char *) "normal", OPT_LEVEL1},
    {(const char *) "all", OPT_DO_ALL},
    {(const char *) NULL, 0}
};

static OPTENUM registeroptions[] =
{
    {(const char *) "none", 0L},
...
{
    {
	(const char *) "collect=", enumeration_option,
    ...
    {
	(const char *) "parallel=", enumeration_option,
	{&opt_parallel_instructions},
	{&threelevelopts[0]}
    },
    {
