...
#ifdef EXTENSION
static TYP *declaration_specifiers P_ ((STORAGE *, STORAGE, VISIBILITY *));
...
static void check_function_declaration P1 (SYM *, sp)
{
	...
	if (!is_void (returned_type (typeof (sp)))) {
	    check_complete (returned_type (typeof (sp)));
	}
	break;
    default:
 ...
 */
#ifdef EXTENSION
static SIZE struct_declarator_list P4 (TYP *, tp, SIZE, offset, TYP *, stp, VISIBILITY, vis)
#else
 ...
 *              visibility ":"
 */
static VISIBILITY visibility_specifier P1 (VISIBILITY, vis)
{
    if (is_visibility_specifier ()) {
	switch (lastst) {
	case kw_public:
	    vis = vi_public;
	    getsym ();
...
static SIZE struct_declaration_list P1 (TYP *, tp)
{
...
#ifdef EXTENSION
    VISIBILITY vis = is_class (tp) ? vi_private : vi_public;
	...
	if (is_class (tp)) {
	    vis = visibility_specifier (vis);
	}
	visible = vis;
	tp1 = declaration_specifiers (&sc, sc_member, &visible);
	size = struct_declarator_list (tp1, offset, tp, visible);
#else
 ...
 */
#ifdef EXTENSION
static TYP *declaration_specifiers P3 (STORAGE *, storage, STORAGE, def_sc, VISIBILITY *, pvis)
#else
...
#ifdef EXTENSION
	if (is_class_scope ()) {
	    sp = tag_search (lastsym);
#if 0
	    if (sp != NIL_SYM || !is_class (typeof (sp))) {
	    }
#endif
	} else
#endif /* EXTENSION */
...
#ifdef EXTENSION
    tp = declaration_specifiers (&sc, def_sc, NULL);
#else
